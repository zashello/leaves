#include <string.h>
#include "ha_mqtt.h"
#include "ha_service.h"
#include "esp_log.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "freertos/event_groups.h"
#include "config/nvs_config.h"

static const char *TAG = "HA_MQTT";

static esp_mqtt_client_handle_t g_mqtt_client = NULL;
static ha_mqtt_state_t g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
static TaskHandle_t g_reconnect_task = NULL;

static void mqttReconnectTask(void *param);

static void mqttEventHandler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    esp_mqtt_event_handle_t event = event_data;
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED:
            ESP_LOGI(TAG, "MQTT已连接");
            g_mqtt_state = HA_MQTT_STATE_CONNECTED;

            esp_mqtt_client_publish(event->client, MQTT_STATE_TOPIC, MQTT_LWT_PAYLOAD_ONLINE, 0, 1, 1);

            ESP_LOGI(TAG, "订阅AI触发主题: %s", MQTT_TRIGGER_AI_TOPIC);
            esp_mqtt_client_subscribe(event->client, MQTT_TRIGGER_AI_TOPIC, 1);
            break;

        case MQTT_EVENT_DISCONNECTED:
            ESP_LOGW(TAG, "MQTT已断开连接");
            g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
            if (g_reconnect_task == NULL) {
                xTaskCreate(mqttReconnectTask, "mqtt_reconn", 4096, NULL, 5, &g_reconnect_task);
            }
            break;

        case MQTT_EVENT_SUBSCRIBED:
            ESP_LOGI(TAG, "已订阅主题: msg_id=%d", event->msg_id);
            break;

        case MQTT_EVENT_UNSUBSCRIBED:
            ESP_LOGI(TAG, "已取消订阅: msg_id=%d", event->msg_id);
            break;

        case MQTT_EVENT_PUBLISHED:
            ESP_LOGD(TAG, "消息已发布: msg_id=%d", event->msg_id);
            break;

        case MQTT_EVENT_DATA:
            ESP_LOGI(TAG, "收到MQTT数据: topic=%.*s, data_len=%d",
                     event->topic_len, event->topic, event->data_len);
            if (event->topic_len <= 128 && event->data_len <= 512) {
                char topic[129] = {0};
                char data[513] = {0};
                memcpy(topic, event->topic, event->topic_len);
                memcpy(data, event->data, event->data_len);

                ha_service_handle_mqtt_data(topic, data, event->data_len);
            }
            break;

        case MQTT_EVENT_ERROR:
            ESP_LOGE(TAG, "MQTT错误");
            break;

        default:
            ESP_LOGD(TAG, "其他MQTT事件: %d", event_id);
            break;
    }
}

static void mqttReconnectTask(void *param)
{
    while (g_mqtt_state != HA_MQTT_STATE_CONNECTED) {
        if (g_mqtt_state == HA_MQTT_STATE_CONNECTING) {
            vTaskDelay(pdMS_TO_TICKS(1000));
            continue;
        }

        g_mqtt_state = HA_MQTT_STATE_RECONNECTING;
        ESP_LOGI(TAG, "尝试重连MQTT服务器...");

        device_config_t config;
        if (configLoad(&config) != ESP_OK || strlen(config.mqttServer) == 0) {
            ESP_LOGW(TAG, "MQTT配置无效，停止重连");
            break;
        }

        esp_mqtt_client_config_t mqtt_cfg = {
            .broker.address.uri = config.mqttServer,
            .credentials = {
                .username = "",
                .authentication = {
                    .password = ""
                }
            },
            .session = {
                .last_will = {
                    .topic = MQTT_STATE_TOPIC,
                    .msg = MQTT_LWT_PAYLOAD_OFFLINE,
                    .qos = 1,
                    .retain = 1
                }
            }
        };

        char uri[300];
        if (strlen(config.mqttUsername) > 0) {
            snprintf(uri, sizeof(uri), "mqtt://%s:%s@%s:%d",
                     config.mqttUsername, config.mqttPassword,
                     config.mqttServer, config.mqttPort);
        } else {
            snprintf(uri, sizeof(uri), "mqtt://%s:%d",
                     config.mqttServer, config.mqttPort);
        }
        mqtt_cfg.broker.address.uri = uri;

        if (g_mqtt_client == NULL) {
            g_mqtt_client = esp_mqtt_client_init(&mqtt_cfg);
            if (g_mqtt_client == NULL) {
                ESP_LOGE(TAG, "MQTT客户端初始化失败");
                vTaskDelay(pdMS_TO_TICKS(HA_MQTT_RECONNECT_DELAY_MS));
                continue;
            }
            esp_mqtt_client_register_event(g_mqtt_client, ESP_EVENT_ANY_ID, mqttEventHandler, NULL);
        } else {
            esp_mqtt_client_set_uri(g_mqtt_client, uri);
        }

        g_mqtt_state = HA_MQTT_STATE_CONNECTING;
        esp_err_t ret = esp_mqtt_client_start(g_mqtt_client);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "MQTT客户端启动失败: %s", esp_err_to_name(ret));
            g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
        }

        vTaskDelay(pdMS_TO_TICKS(HA_MQTT_RECONNECT_DELAY_MS));
    }

    g_reconnect_task = NULL;
    vTaskDelete(NULL);
}

esp_err_t ha_mqtt_init(void)
{
    if (g_mqtt_client != NULL) {
        ESP_LOGW(TAG, "MQTT客户端已初始化");
        return ESP_OK;
    }

    device_config_t config;
    esp_err_t ret = configLoad(&config);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "加载配置失败");
        return ret;
    }

    if (strlen(config.mqttServer) == 0) {
        ESP_LOGW(TAG, "MQTT服务器未配置，跳过MQTT初始化");
        return ESP_OK;
    }

    ESP_LOGI(TAG, "初始化MQTT客户端: %s:%d", config.mqttServer, (int)config.mqttPort);

    char uri[300];
    if (strlen(config.mqttUsername) > 0) {
        snprintf(uri, sizeof(uri), "mqtt://%s:%s@%s:%d",
                 config.mqttUsername, config.mqttPassword,
                 config.mqttServer, config.mqttPort);
    } else {
        snprintf(uri, sizeof(uri), "mqtt://%s:%d",
                 config.mqttServer, config.mqttPort);
    }

    esp_mqtt_client_config_t mqtt_cfg = {
        .broker.address.uri = uri,
        .session = {
            .last_will = {
                .topic = MQTT_STATE_TOPIC,
                .msg = MQTT_LWT_PAYLOAD_OFFLINE,
                .qos = 1,
                .retain = 1
            }
        }
    };

    g_mqtt_client = esp_mqtt_client_init(&mqtt_cfg);
    if (g_mqtt_client == NULL) {
        ESP_LOGE(TAG, "MQTT客户端初始化失败");
        return ESP_FAIL;
    }

    esp_mqtt_client_register_event(g_mqtt_client, ESP_EVENT_ANY_ID, mqttEventHandler, NULL);

    ESP_LOGI(TAG, "MQTT客户端初始化成功");
    return ESP_OK;
}

esp_err_t ha_mqtt_deinit(void)
{
    if (g_mqtt_client != NULL) {
        ha_mqtt_publish(MQTT_STATE_TOPIC, MQTT_LWT_PAYLOAD_OFFLINE, 1, 1);
        esp_mqtt_client_stop(g_mqtt_client);
        esp_mqtt_client_destroy(g_mqtt_client);
        g_mqtt_client = NULL;
    }

    g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
    ESP_LOGI(TAG, "MQTT客户端已清理");
    return ESP_OK;
}

esp_err_t ha_mqtt_connect(void)
{
    if (g_mqtt_client == NULL) {
        ESP_LOGE(TAG, "MQTT客户端未初始化");
        return ESP_ERR_INVALID_STATE;
    }

    if (g_mqtt_state == HA_MQTT_STATE_CONNECTED || g_mqtt_state == HA_MQTT_STATE_CONNECTING) {
        ESP_LOGW(TAG, "MQTT已连接或正在连接");
        return ESP_OK;
    }

    ESP_LOGI(TAG, "启动MQTT客户端");
    g_mqtt_state = HA_MQTT_STATE_CONNECTING;
    esp_err_t ret = esp_mqtt_client_start(g_mqtt_client);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "MQTT客户端启动失败: %s", esp_err_to_name(ret));
        g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
        return ret;
    }

    int retry = 0;
    const int max_retry = HA_MQTT_CONNECT_TIMEOUT_MS / 1000;
    while (g_mqtt_state != HA_MQTT_STATE_CONNECTED && retry < max_retry) {
        vTaskDelay(pdMS_TO_TICKS(1000));
        retry++;
    }

    if (g_mqtt_state != HA_MQTT_STATE_CONNECTED) {
        ESP_LOGE(TAG, "MQTT连接超时");
        esp_mqtt_client_stop(g_mqtt_client);
        g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
        return ESP_ERR_TIMEOUT;
    }

    return ESP_OK;
}

esp_err_t ha_mqtt_disconnect(void)
{
    if (g_mqtt_client == NULL) {
        return ESP_OK;
    }

    ha_mqtt_publish(MQTT_STATE_TOPIC, MQTT_LWT_PAYLOAD_OFFLINE, 1, 1);
    esp_mqtt_client_stop(g_mqtt_client);
    g_mqtt_state = HA_MQTT_STATE_DISCONNECTED;
    ESP_LOGI(TAG, "MQTT已断开");
    return ESP_OK;
}

bool ha_mqtt_is_connected(void)
{
    return g_mqtt_state == HA_MQTT_STATE_CONNECTED;
}

ha_mqtt_state_t ha_mqtt_get_state(void)
{
    return g_mqtt_state;
}

esp_err_t ha_mqtt_publish(const char *topic, const char *data, int qos, int retain)
{
    if (g_mqtt_client == NULL || ha_mqtt_is_connected() == false) {
        ESP_LOGW(TAG, "MQTT未连接，跳过发布: %s", topic);
        return ESP_ERR_INVALID_STATE;
    }

    int msg_id = esp_mqtt_client_publish(g_mqtt_client, topic, data, strlen(data), qos, retain);
    if (msg_id < 0) {
        ESP_LOGE(TAG, "发布失败: topic=%s", topic);
        return ESP_FAIL;
    }

    ESP_LOGD(TAG, "发布消息: topic=%s, msg_id=%d", topic, msg_id);
    return ESP_OK;
}

esp_err_t ha_mqtt_subscribe(const char *topic, int qos)
{
    if (g_mqtt_client == NULL) {
        return ESP_ERR_INVALID_STATE;
    }

    int msg_id = esp_mqtt_client_subscribe(g_mqtt_client, topic, qos);
    if (msg_id < 0) {
        ESP_LOGE(TAG, "订阅失败: topic=%s", topic);
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "订阅主题: topic=%s, msg_id=%d", topic, msg_id);
    return ESP_OK;
}

esp_err_t ha_mqtt_set_callback(esp_mqtt_client_handle_t client)
{
    if (client != NULL) {
        esp_mqtt_client_register_event(client, ESP_EVENT_ANY_ID, mqttEventHandler, NULL);
        return ESP_OK;
    }
    return ESP_ERR_INVALID_ARG;
}
