...
{
    if (gpio_get_level(RESET_BUTTON_GPIO) == BUTTON_ACTIVE_LEVEL) {
        ESP_LOGW(TAG, "长按3秒触发");
        if (gCallback) {
...
static void debounceCallback(TimerHandle_t xTimer)
{
    ...
    if (level == BUTTON_ACTIVE_LEVEL && !gButtonPressed) {
        gButtonPressed = true;
        ESP_LOGI(TAG, "按键按下，开始长按计时...");
        xTimerStart(gLongPressTimer, 0);
    } else if (level != BUTTON_ACTIVE_LEVEL && gButtonPressed) {
...
esp_err_t buttonInit(void)
{
    ...
    ret = gpio_isr_handler_add(RESET_BUTTON_GPIO, buttonIsrHandler, NULL);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "ISR注册失败: %s", esp_err_to_name(ret));
        return ret;
    }

    ESP_LOGI(TAG, "按键初始化成功 (GPIO%d, 长按%ds触发重置)", RESET_BUTTON_GPIO, BUTTON_LONG_PRESS_MS / 1000);
    return ESP_OK;
}
