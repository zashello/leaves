...
static esp_err_t saveHandler(httpd_req_t *req)
{
    ...
    if (item && cJSON_IsString(item)) {
        strncpy(config.wifiSsid, item->valuestring, sizeof(config.wifiSsid) - 1);
    }

    item = cJSON_GetObjectItem(root, "password");
    if (item && cJSON_IsString(item)) {
        strncpy(config.wifiPass, item->valuestring, sizeof(config.wifiPass) - 1);
    }

    item = cJSON_GetObjectItem(root, "sfKey");
    if (item && cJSON_IsString(item)) {
        strncpy(config.siliconflowKey, item->valuestring, sizeof(config.siliconflowKey) - 1);
    }
    ...
    esp_err_t err = configSave(&config);
    if (err != ESP_OK) {
        httpd_resp_set_type(req, "application/json");
